...
public class Simulation {
    ...
    public void simulate(){
        ...
        for(int i = 1; i <= numberOfDays; i++) {
            DaySimulation simulateDay = new DaySimulation(passengers, tramLines, stops, numberOfDays);
            simulateDay.simulateDay();
            data[i-1] = new SimulationDayStatistics(stops, passengers);
            System.out.println("Day: "+ i +"\nSTATISTICS\n" + data[i-1].toString());
            mainStatistics.addDay(data[i-1]);
        }
        System.out.println(mainStatistics.toString());
        for(int i = 1; i <= numberOfDays; i++) {
            System.out.println("Day: "+ i +"\nSTATISTICS\n" + data[i-1].toString());
        }
...
//            }
//        }
//        SimulationDayStatistics statistics = new SimulationDayStatistics(stops, passengers);
//        System.out.println( statistics.toString());
    ...
    private void scanInput(){
        ...
        for(int i = 0; i < numberOfPassengers; i++){
            passengers[i] = new Passenger(i, stops[Losowanie.losuj(0, stops.length - 1)]);
        }
