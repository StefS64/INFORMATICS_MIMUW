...
.parse_string_poly_crc:				; Parsuje podanego stringa i zapisuje go w rax
	...
	sub bl, '0'
	cmp bl, 1
	ja .input_crc_poly_error		; Sprawdza poprawnosc 0 <= bl <= 1 (czytamy unsigned)
...
.fragment_loop:
	...
	jb .read_failed
   										...
   										; Wczytujemy fragment do obliczenia
   lea rsi, [rel buffer]			;
   mov rdi, [rel file_descriptor];
   movzx rdx, word [rel block_length]		
.not_all_read_again:					; Jeżeli nie wszystko wczytane kontynujemy
   mov rax, SYS_READ				   	
	syscall
	test rax, rax
	js .read_failed
...
.finished_fragment_crc:
	...
	mov rax, SYS_READ				;
	mov rdi, [rel file_descriptor]	   	
	lea rsi, [rel shift]			; Zapisujemy 4 bajtowy fragment skoku
	mov rdx, 4			         ;
	syscall
   cmp rax, 4
	jb .read_failed
...
.bit_is_0_2:
	...
	jnz .zero_loop
 	...
 	mov rcx, crc_poly_out ; Pointer na koniec fragmentu
