...
%%begin:
  ...
  mov     eax, 1                  ; SYS_WRITE
  mov     edi, 1                  ; STDOUT
  lea     rsi, [rel %%descr]      ; Wskaźnik do napisu (wyjątkowo w sekcji .text).
  mov     rdx, %%begin - %%descr  ; Długość napisu.
  syscall
...
%%loop:
	movzx rax, dl
	and al, 0x0f ; zerujemy wyższe bity dolne zostawiamy
	mov rax, [%%ascii_digits + al]
	mov [rsp + 55 + rcx], al
	shr rdx, 4
	loop %%loop


  ; Dopisz na koniec bufora znak końca linii (nadpisujemy [rsp + 72], nie jest już potrzebne).
  mov [rsp + 56 + 16], byte `\n`

  ; Syscall write(STDOUT, bufor, długość 17).
  mov     eax, 1          ; SYS_WRITE
  mov     edi, 1          ; STDOUT
  lea     rsi, [rsp + 56] ; Wskaźnik do napisu.
  mov     edx, 17         ; Długość napisu 16 znaków + 1 końca linii
  syscall
