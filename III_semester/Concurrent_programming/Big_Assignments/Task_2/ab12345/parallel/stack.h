...
inline bool should_stop(Stack s) { // merge with stop
...
inline void init(Stack* s, int capacity, int num_threads) {
...
inline void push(Stack s, elem_state elem) {
...
inline void pop(Stack s, elem_state *elem) {
    ASSERT_ZERO(pthread_mutex_lock(&s->mutex));
    #ifdef COUNT
    pop_counter++;
    printf("popped: %d\n", pop_counter);
