...
/*
   Funkcja pre_proces zwraca minimalną liczbę ruchów by mieć pewność odgadnięcia odpowiedzi z aktualnego stanu wektora,
   (komórka vecotra zawiera ilość kłamstw których Ala musiałaby powiedzieć by nie być pewien czy ta liczba jest jedną z możliwości dobrego wyniku) 
   Przy okazji wrzuca najbardziej optymalny ruch z stanu aktualnego do mapy.
*/

int pre_proces(vector<int> &aktualny_stan, int zap, bool odpowiedz){//stan wektora indeks zapytania oraz odpowiedz twierdzaca lub nie na zapytaniw
...
void zagraj(vector<int> stan){
   if(mapa[stan].first == 0){
      ...
      if(mniejszaNiz(mapa[stan].second)){//wybiera, optymalny ruch korzystając z pre-procesowanych wektorów
