...
void NetworkManager::handleIncomingMessage() {
  ...
  switch (message_type) {
...
#endif 
    if (_getFlag(sender_addr) == UNCONFIRMED && len == 1) { 
      addConnection(sender_addr, CONFIRMED);
    } else {
      error_msg(buffer, len);
    }
    ...
    std::cout << "Received DELAY_REQUEST from "
      ...
      << ntohs(sender_addr.sin_port) << "\n";
#endif
    handleDelayRequest(sender_addr, len);
    break;
    case DELAY_RESPONSE:
#ifdef DEBUG
...
// Father node.
void NetworkManager::handleDelayRequest(const sockaddr_in& sender_addr, ssize_t len) {
  short flag = _getFlag(sender_addr);
  if (flag != SYNCHRONIZING || !checkTime(sender_addr) || len != 1) {
#ifdef DEBUG
...
void NetworkManager::handleLeader(ssize_t len) {
  ...
  if (state != LEADING && flag == 0) {
    data.setSyncLevel(0);
    set_leader_time = GlobalClock::now();
    state = LEADING;
    return;
  }
...
void NetworkManager::handleSyncSending() {
  ...
  if (GlobalClock::now() - set_leader_time < TWO_SECONDS) {
    return;
  }
